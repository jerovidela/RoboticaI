...
for i = 1:num_lines
    ...
    if N > 0
        ...
        % Trayectoria cartesiana recta + refinamiento por IK 
...
fprintf('Trayectoria computada.'); pause(3); clc;

out = false;

while ~out
    prompt = "¿Mostrar animación? [s/n]:  ";
...
T_tmp = R.fkine(Q);        % SE3 array (RTB10) o 4x4xN (RTB9)
s_pos = zeros(N, 3);

for k = 1:N, s_pos(k, :) = transl(T_tmp(k)); end
...
if subplot_pos
    figure('Name','Posición cartesiana del TCP');
    subplot(3,1,1); plot(t, s_pos(:,1)); grid on;  xlabel('t [s]'); ylabel('Sx [m]');
    subplot(3,1,2); plot(t, s_pos(:,2)); grid on; xlabel('t [s]'); ylabel('Sy [m]');
    subplot(3,1,3); plot(t, s_pos(:,3)); grid on; xlabel('t [s]'); ylabel('Sz [m]');
else
    C = [0.00 0.45 0.74;   % X
     0.85 0.33 0.10;   % Y
     0.47 0.67 0.19];  % Z

    figure('Name','Perfil de posición cartesiana','Color','w');
    hold on; grid on; box off;
    
    plot(t, s_pos(:,1), 'LineWidth', 1.8, 'Color', C(1,:), 'DisplayName','Coordenada X');
    plot(t, s_pos(:,2), 'LineWidth', 1.8, 'Color', C(2,:), 'DisplayName','Coordenada Y');
    plot(t, s_pos(:,3), 'LineWidth', 1.8, 'Color', C(3,:), 'DisplayName','Coordenada Z');
    
    xlabel('t [s]');
    ylabel('Posición [m]');
    title('Perfil de posición cartesiana');
...
if subplot_vel
    figure('Name','Velocidad cartesiana del TCP');
    subplot(4,1,1); plot(t, v(:,1)); grid on;  xlabel('t [s]'); ylabel('Vx [m/s]');
    subplot(4,1,2); plot(t, v(:,2)); grid on; xlabel('t [s]'); ylabel('Vy [m/s]');
    subplot(4,1,3); plot(t, v(:,3)); grid on; xlabel('t [s]'); ylabel('Vz [m/s]');
    ...
    plot(t, v(:,1), 'LineWidth', 1.8, 'Color', C(1,:)); grid on; xlabel('t [s]'); ylabel('Vx [m/s]');
    
    figure('Name','perfil velocidad cartesiana Y');
    plot(t, v(:,2), 'LineWidth', 1.8, 'Color', C(2,:)); grid on; xlabel('t [s]'); ylabel('Vy [m/s]');
    
    figure('Name','perfil velocidad cartesiana Z');
    plot(t, v(:,3), 'LineWidth', 1.8, 'Color', C(3,:)); grid on; xlabel('t [s]'); ylabel('Vz [m/s]');
    
    figure('Name','error de velocidad (módulo)');
    plot(t, vnorm,  'LineWidth', 1.8, 'Color', [1 0 0]);  grid on; xlabel('t [s]'); ylabel('||V||'); xlabel('t [s]');
end
...
if subplot_acc
    ...
    plot(t, a(:,1), 'LineWidth', 1.8, 'Color', C(1,:), 'DisplayName','Aceleración X');
    plot(t, a(:,2), 'LineWidth', 1.8, 'Color', C(2,:), 'DisplayName','Aceleración Y');
    plot(t, a(:,3), 'LineWidth', 1.8, 'Color', C(3,:), 'DisplayName','Aceleración Z');
    
    xlabel('t [s]');
    ylabel('Aceleración [m/s²]');
    title('Perfil de aceleración cartesiana');
    legend('Location','best','Box','off');
    axis tight;
    hold off;
else
    figure('Name','perfil de aceleración cartesiana  X');
    plot(t, a(:,1), 'LineWidth', 1.8, 'Color', C(1,:)); grid on; xlabel('t [s]'); ylabel('Ax [m/s^2]');
    
    figure('Name','perfil de aceleración cartesiana  Y');
    plot(t, a(:,2), 'LineWidth', 1.8, 'Color', C(2,:)); grid on; xlabel('t [s]'); ylabel('Ay [m/s^2]');
    
    figure('Name','perfil de aceleración cartesiana  Y');
    plot(t, a(:,3), 'LineWidth', 1.8, 'Color', C(3,:)); grid on; xlabel('t [s]'); ylabel('Az [m/s^2]');
