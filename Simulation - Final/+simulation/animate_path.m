...
vw = [];  % evita 'Unrecognized function or variable vw'

% ---------- defaults seguros, sin isstruct ----------
if ~builtin('isfield', cfg, 'fps'),         cfg.fps = 30;              end
if ~builtin('isfield', cfg, 'trail_color'), cfg.trail_color = [0 0 0]; end
if ~builtin('isfield', cfg, 'trail_width'), cfg.trail_width = 1.5;     end
if ~builtin('isfield', cfg, 'ws'),          cfg.ws = [-0.5 0.8 -0.5 0.8 0 1]; end

% Asegurar cfg.plate
if ~builtin('isfield', cfg, 'plate')
    cfg.plate = struct();
else
    try
        builtin('isfield', cfg.plate, 'center');
    catch
        cfg.plate = struct();
    end
end
if ~builtin('isfield', cfg.plate, 'center'), cfg.plate.center = [0.45 0.20]; end
if ~builtin('isfield', cfg.plate, 'size'),   cfg.plate.size   = [0.40 0.30]; end
if ~builtin('isfield', cfg.plate, 'color'),  cfg.plate.color  = [0.72 0.75 0.78]; end

if ~builtin('isfield', cfg, 'save_video'), cfg.save_video = false; end
if ~builtin('isfield', cfg, 'video_file'), cfg.video_file = 'scan.mp4'; end
% ---------- fin defaults ----------
    ...
    if cfg.save_video
        ...
        [profile, cfg.video_file] = pick_video_profile(cfg.video_file);
        vw = VideoWriter(cfg.video_file, profile);
        vw.FrameRate = cfg.fps;
        % En AVI se puede subir un poco la calidad
        try
...
% ---------- Helpers ----------
function [profile, outfile] = pick_video_profile(outfile)
    % Devuelve perfil válido y corrige la extensión de archivo según soporte
    profile = 'Motion JPEG AVI';  % default seguro

    try
        profs = VideoWriter.getProfiles();
        names = string({profs.Name});
    catch
        names = string("Motion JPEG AVI");
    end

    is_mp4_supported = any(names == "MPEG-4");

    if isstring(outfile), outfile = char(outfile); end

    if ispc && is_mp4_supported
        profile = 'MPEG-4';
        if ~endsWith(lower(outfile), '.mp4')
            outfile = [regexprep(outfile, '\.(avi|mov|m4v)$','', 'ignorecase') '.mp4'];
        end
    else
        if ~endsWith(lower(outfile), '.avi')
            outfile = [regexprep(outfile, '\.(mp4|mov|m4v)$','', 'ignorecase') '.avi'];
        end
