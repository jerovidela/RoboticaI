...
function d = derivada(vz, t)
    ...
    if m <= 2; warning('Pocos puntos para derivada.'); d = zeros(m, n); return; end
    if length(t) ~= m; error('Tiempo y datos no coinciden.'); end
    dt_vec = diff(t); dt_mean = mean(dt_vec);
    d=zeros (m, n);
    if any(abs(dt_vec - dt_mean) > 1e-6 * abs(dt_mean)) % Tiempo no constante
       for j=1:n
           d(1,j) = (-3*vz(1,j) + 4*vz(2,j) - vz(3,j)) / (t(3)-t(1));
           for i=2:m-1; d(i,j) = (vz(i+1,j) - vz(i-1,j)) / (t(i+1)-t(i-1)); end
           d(m,j) = (3*vz(m,j) - 4*vz(m-1,j) + vz(m-2,j)) / (t(m)-t(m-2));
       end
    else % Tiempo constante
       dt = dt_mean; if abs(dt) < eps; warning('dt casi cero'); return; end
       for j=1:n
           d(1,j) = (-3*vz(1,j) + 4*vz(2,j) - vz(3,j)) / (2*dt);
           for i=2:m-1; d(i,j) = (vz(i+1,j) - vz(i-1,j)) / (2*dt); end
